(() => {
	var n = {
			...
			446: (n, e, t) => {
				...
				if (((i.regeneratorRuntime = void 0), t(989), o))
			...
			612: () => {
				...
				((window.initPokiBridge = function (e) {
					((n = e),
						window.unityGame
							? window.pokiReady || window.pokiAdBlock
								? window.pokiReady
							...
							(C.prototype = {
								...
								finish: function (n) {
									for (var e = this.tryEntries.length - 1; e >= 0; --e) {
										...
										if (t.finallyLoc === n)
											return (this.complete(t.completion, t.afterLoc), T(t), f);
									}
								},
								catch: function (n) {
									for (var e = this.tryEntries.length - 1; e >= 0; --e) {
								...
								return function (o, r) {
									...
									for (t.method = o, t.arg = r; ; ) {
										...
										if ("next" === t.method) t.sent = t._sent = t.arg;
											...
											if (i === c) throw ((i = m), t.arg);
											t.dispatchException(t.arg);
										} else "return" === t.method && t.abrupt("return", t.arg);
										i = u;
										var l = b(n, e, t);
										if ("normal" === l.type) {
											if (((i = t.done ? m : h), l.arg === f)) continue;
											return { value: l.arg, done: t.done };
										}
										"throw" === l.type &&
											((i = m), (t.method = "throw"), (t.arg = l.arg));
									}
								};
							})(n, t, a)),
							r
	...
	function t(i) {
		...
		return (n[i](r, r.exports, t), r.exports);
	}
	((t.n = (n) => {
		...
		(() => {
			...
			const p = Object.assign(
				{
					...
					minRatio: 9 / 16,
					thumbnail:
						"https://img.poki-cdn.com/cdn-cgi/image/quality=80,width=100,height=100,fit=cover,g=0.5x0.5,f=auto/a398cd4e645b76810031bd97f8697414.png",
					numScreenshots: 4,
			...
			const L = p.screenshotsVersion ? `?v${p.screenshotsVersion}` : "",
				I = window.location.pathname.substring(
					0,
			...
			async function T() {
				...
				if (e.getAttribute("fullImageLoaded"))
					...
					r > n
						? C()
						: (window.slideShowTimeoutID = window.setTimeout(C, n - r));
				}
			}
			function C() {
				if (E) return;
					...
					(function (n) {
						...
						y.querySelectorAll(".image").forEach((n) => {
							(n.className === `${b} left` && (n.className = `${b} fromLeft`),
								n.className === `${b} right` &&
									(n.className = `${b} fromRight`),
								-1 === n.className.indexOf("inactive") &&
			...
			function F() {
				...
				e > p.fullImageMaxHeight &&
					((e = p.fullImageMaxHeight), (t = (e * x) / n));
			...
			function z() {
				...
				((M.style.width = `${n}px`),
					(M.style.height = `${e}px`),
					t > p.maxRatio
						? (M.style.width = e * p.maxRatio + "px")
						: t < p.minRatio && (M.style.height = n / p.minRatio + "px"));
				const i = M.getBoundingClientRect();
				((M.style.marginLeft = -0.5 * i.width + "px"),
					(M.style.marginTop = -0.5 * i.height + "px"));
			}
			function _(n) {
				const e = 100 * n;
				((D.style.width = `${e}%`),
			...
			function H() {
				...
				n
					? ((j.innerHTML = n[G]),
						G++,
						G >= n.length && (G = 0),
						(P = setTimeout(H, p.commentChangeTime)))
					: (j.innerHTML = "");
			}
			function q() {
						...
						(async function () {
							...
							const i = (n) => {
								...
								((t.src = `${$}screenshots/${n + 1}-small.jpg${L}`),
									...
									1 === n
										? (e.className = `${b} right`)
										: n === p.numScreenshots - 1
											? (e.className = `${b} left`)
											: (e.className = `${b} inactive`),
									y.appendChild(e));
							};
							for (let n = 1; n <= p.numScreenshots - 1; n++) i(n);
							T();
						})().then(() => {
							U();
						});
					} catch (n) {
						(console.info("Slideshow loading error", n), U());
					}
					q();
				}));
			t(612);
			...
			((V.id = "thumbnail"), (V.alt = p.title), (V.title = p.title));
			let J = p.thumbnail;
			const Q = "https://img.poki-cdn.com/";
			(J.startsWith(Q) &&
				(J = `${Q}cdn-cgi/image/quality=78,width=40,height=40,fit=cover,g=0.5x0.5,f=auto/${J.substr(25)}`),
				(V.src = J));
